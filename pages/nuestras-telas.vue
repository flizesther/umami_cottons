<template>
  <b-container class="telas">
    ...
    <div v-for="fabric in fabrics" :key="fabric.id" class="d-flex flex-column align-items-center">
      <b-img class="fabric-image d-flex w-25" :src="fabric.image" :alt="fabric.name"> </b-img>
      <b-row class="d-flex fabrics-container">
        <div v-for="imageFabric in fabric" :key="imageFabric.id">
          <b-img class="fabric-image d-flex" :src="imageFabric.image" :alt="imageFabric.name">{{
            imageFabric.image
          }}</b-img>
        </div>
      </b-row>
...
export default {
  ...
  computed: {
    imageTelas() {
      return this.fabrics.map((f) => f.id).map((i) => i.image)
    },
  },
